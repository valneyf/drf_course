...
class ContactTestCase(APITestCase):
    ...
    def test_create_contact(self):
        ...
        self.assertEqual(Contact.objects.get().title, 'Valney Filho')

    def test_create_contact_without_name(self):
        '''
        test ContactViewSet create method when name is not is data
        '''
        data = self.data
        data.pop('name')
        response = self.client.post(self.url, data)
    ...
    def test_create_contact_when_message_equals_blank(self):
        '''
        test ContactViweSet create method when message is blank
        '''
        data = self.data
        data['message'] = ''
        response = self.client.post(self.url, data)
